...
class AICaller:
    ...
    def call_model(self, prompt, max_tokens=4096):
        ...
        response = litellm.completion(model=self.model, api_base=self.api_base, messages=messages, max_tokens=max_tokens, stream=True) if "ollama" in self.model or "huggingface" in self.model else litellm.completion(model=self.model, messages=messages, max_tokens=max_tokens, stream=True)
